 ...
 * (e.g., "https://ediths.example.com"). During local development, it falls
...
/*
 * currentMode — Which AI pipeline to use: 'edith', 'general', or 'realtime'.
 * - edith:   Unified route; brain classifies, then routes to general or realtime.
 * - general:  Direct /chat/stream (no web search).
 * - realtime: Direct /chat/realtime/stream (with Tavily web search).
 */
let currentMode = 'edith';
 ...
 */
const SETTINGS_KEY = 'edith_settings';
...
const btnEdith = $('btn-edith');      // The "Edith" mode button (unified, brain-routed)
...
const activityPanel = $('activity-panel');          // Left panel for Edith activity flow
...
function init() {
    if (!chatMessages || !messageInput) {
        console.error('[EDITH] Required DOM elements (chat-messages, message-input) not found.');
        return;
    }
    loadSettings();
 ...
 */
function showToast(msg, durationMs = 5000, extraClass = '') {
...
function bindEvents() {
    ...
    // MODE TOGGLE BUTTONS — Switch between Edith, General, and Realtime modes
    if (btnEdith) btnEdith.addEventListener('click', () => setMode('edith'));
...
function setMode(mode) {
    currentMode = mode;
    console.log('[EDITH] MODE:', mode);
    if (!btnEdith) console.warn('[EDITH] btnEdith is null — id="btn-edith" not found in HTML');
    if (btnEdith) btnEdith.classList.toggle('active', mode === 'edith');
    ...
    if (modeSlider) {
        ...
        /* edith: no class = left position (slider at start) */
    }
    // Activity toggle: always visible so user can open the panel
...
function newChat() {
    ...
    updatePanelOverlay();
    setMode(currentMode);   // Resync mode buttons, slider, and activityToggle visibility
}
...
/* ================================================================
   ...
   consists of:
     - An avatar circle ("E" for Edith , "U" for user)
...
function renderSearchResults(payload) {
    ...
    if (searchResultsQuery) searchResultsQuery.textContent = 'Live Search Sources';
 ...
 *     <div class="msg-body">
 *       <div class="msg-label">Edith (General) | You</div>
...
function addMessage(role, text) {
    ...
    const labelMap = { edith: 'Edith', general: 'Edith (General)', realtime: 'Edith (Realtime)' };
    const label = document.createElement('div');
    label.className = 'msg-label';
    label.textContent = role === 'assistant'
        ? (labelMap[currentMode] || 'Edith')
        : 'You';
...
function addTypingIndicator() {
    ...
    const label = document.createElement('div');
    label.className = 'msg-label';
    label.textContent = labelMap[currentMode] || 'Edith';
...
async function sendMessage(textOverride) {
    ...
    const endpointMap = {
        edith: '/chat/jarvis/stream',
        general: '/chat/stream',
        realtime: '/chat/realtime/stream',
    };
    const endpoint = endpointMap[currentMode] || '/chat/jarvis/stream';
    console.log('ENDPOINT:', endpoint);   // Debug: verify correct route is used
    ...
    try {
        ...
        while (!streamDone) {
            ...
            for (const line of lines) {
                ...
                try {
                    ...
                    // ACTIVITY — Edith flow (query detected, decision, routing): show in left panel
                    if (data.activity) {
        ...
        clearTimeout(timeoutId);
        removeTypingIndicator();
        const msg = err.name === 'AbortError' ? 'Request timed out.' : `Request failed: ${err.message}`;
...
/* ================================================================
   UPLOAD MODULE — Knowledge ingestion via /upload endpoint
   ================================================================
   Allows the user to upload a .txt file OR paste raw text.
   Sends to POST /upload with multipart/form-data.
   Uses the existing `sessionId` global and `showToast()`.
   Completely isolated — never touches sendMessage, SSE, or TTS.
   ================================================================ */

document.addEventListener('DOMContentLoaded', function initUploadModule() {
    // ── DOM refs ──────────────────────────────────────────────────
    ...
    const pasteTextarea   = document.getElementById('upload-paste-textarea');

    if (!uploadBtn || !uploadModal) return; // guard: elements must exist
    ...
    // ── Upload button click → open modal or file picker ──────────
    uploadBtn.addEventListener('click', () => {
        openUploadModal();
    });

    // ── File input wired to a "browse" button inside the modal ────
    // The hidden <input> is triggered from inside the modal via the
    // .upload-file-btn label (injected below) or programmatically.
    fileInput.addEventListener('change', () => {
        if (fileInput.files && fileInput.files[0]) {
            selectedFile = fileInput.files[0];
            pasteTextarea.value = ''; // clear paste when file chosen
            // Show filename in the modal hint
            const hint = uploadModal.querySelector('.upload-modal-hint');
            if (hint) hint.textContent = `File selected: ${selectedFile.name}`;
        }
    });

    // Inject the "browse file" row into the modal body
    const modalBody = uploadModal.querySelector('.upload-modal-body');
    if (modalBody) {
        const fileRow = document.createElement('div');
        fileRow.className = 'upload-file-row';
        fileRow.innerHTML = `
            <span class="upload-file-label">Or choose a .txt file:</span>
            <button class="upload-file-btn" id="upload-file-btn" type="button">Browse</button>
        `;
        // Insert after hint, before textarea
        const textarea = modalBody.querySelector('.upload-textarea');
        if (textarea) modalBody.insertBefore(fileRow, textarea);
    ...
    // ── API call ──────────────────────────────────────────────────
    async function uploadData({ file, text }) {
        const formData = new FormData();
        if (file) formData.append('file', file);
        if (text) formData.append('text', text.trim());
        // Use the global sessionId if one exists
    ...
    submitBtn.addEventListener('click', async () => {
        ...
        if (!selectedFile && !pastedText) {
            pasteTextarea.focus();
            pasteTextarea.style.borderColor = 'rgba(255, 107, 107, 0.5)';
            setTimeout(() => { pasteTextarea.style.borderColor = ''; }, 1500);
            return;
        }
        ...
        // Show learning toast
        showToast('EDITH is learning…', 6000, 'toast-info');
        ...
        try {
            ...
            closeUploadModal();
            showToast('Knowledge stored successfully', 4000, 'toast-success');
